...
class PollStorage:
    ...
    async def end_poll(self, message_id: int):
        async with (await self._pool()) as pool, pool.acquire() as conn:
            await conn.execute("UPDATE public.polls SET active = FALSE WHERE message_id = $1", message_id)
            await conn.execute("DELETE FROM public.poll_votes WHERE poll_message_id = $1", message_id)
